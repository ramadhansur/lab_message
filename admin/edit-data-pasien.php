<?php 
  ...
  $data_pasien = mysqli_query($koneksi,"SELECT * FROM tb_pasien WHERE nama = '$nama' AND DAY(date) = '$tanggal' AND MONTH(date) = '$bulan'");
  
  // Data akun
  $sql_akun = mysqli_query($koneksi,"SELECT * FROM tb_akun WHERE nama = '$nama' AND no_hp = '$notelp'");
  $data_akun = mysqli_fetch_array($sql_akun);

  $sql = mysqli_query($koneksi,"SELECT * FROM tb_pasien WHERE nama = '$nama' AND DAY(date) = '$tanggal' AND MONTH(date) = '$bulan'");
  $data = mysqli_fetch_array($sql);
  $datacheck = explode(',', $data['pemeriksaan']);
?>
<!DOCTYPE html>
...
<head>
  ...
  <title>
   Edit Data Pasien - Admin Control
  </title>
  <!--     Fonts and icons     -->
  ...
  <main class="main-content position-relative border-radius-lg ">
    ...
    <nav class="navbar navbar-main navbar-expand-lg px-0 mx-4 shadow-none border-radius-xl " id="navbarBlur" data-scroll="false">
      <div class="container-fluid py-1 px-3">
          <p class="font-weight-bolder text-blue page-title">Edit Pasien</p>
            ...
            <div class="card mb-4 p-4">
              <div class="card-body px-0 pt-0 pb-2">
                <div class="table-responsive p-0 ">
                  <form action="./proses/proses-edit-pasien.php" method="post">
                    <input type="text" name="id-akun" value="<?= $data_akun['id'] ?>" hidden>
                    <?php 
                    ...
                    <input class="form-control mb-3" type="text" name="nama" autocomplete="off" value="<?= $data['nama'] ?>">
                    <h4 class="nama-head text-blue ms-0">Email</h4>
                    <input class="form-control mb-3" type="text" name="email" autocomplete="off" value="<?= $data['email'] ?>">
                    <h4 class="nama-head text-blue ms-0">No Telp</h4>
                    <input class="form-control mb-3" type="text" name="notelp" required autocomplete="off" value="<?= $data['no_telp'] ?>">
                    ...
                    <div class="mb-3">
                      <select name="jeniskelamin" id="" class="form-select">
                        ...
                        <option value="Tidak Disebutkan" <?php echo $data['jenis_kelamin'] == "Tidak Disebutkan" ? "selected" : "" ?>>Tidak Disebutkan</option>
                      </select>
                    </div>
          ...
          <div class="row align-items-center justify-content-lg-between">
            <div class="col-lg-6 mb-lg-0 mb-4">
              <div class="copyright text-center text-sm text-muted text-lg-end">
                Copyright © 
                <a href="#" class="font-weight-lighter" align="center">Laboratoium.</a>
                <script>
